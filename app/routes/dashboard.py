...
def overview():
    ...
    if cached_data is None:
        cached_data = calculate_dashboard_data()
    ...
    if cached_data is None:
        return render_template('dashboard.html',
                             ...
                             top_volume_trades=[],
                             top_price_trades=[],
                             trend={'labels': ['01月','02月','03月','04月','05月','06月'], 'issued': [0,0,0,0,0,0], 'sold': [0,0,0,0,0,0]},
                             main_projects=[],
    ...
    return render_template('dashboard.html',
                         ...
                         top_provinces=cached_data.get('top_provinces', []),
                         top_buyer_provinces=cached_data.get('top_buyer_provinces', []),
                         top_secondary_units=cached_data.get('top_secondary_units', []),
                         top_volume_trades=cached_data.get('top_volume_trades', []),
                         top_price_trades=cached_data.get('top_price_trades', []),
                         trend=cached_data.get('trend', {'labels': ['01月','02月','03月','04月','05月','06月'], 'issued': [0,0,0,0,0,0], 'sold': [0,0,0,0,0,0]}),
                         main_projects=cached_data.get('main_projects', []),
                         cache_info=get_cache_info())


@dashboard_bp.route('/api/refresh-cache', methods=['POST'])
@login_required
def refresh_cache():
    """手动刷新dashboard缓存的API接口"""
    try:
        # 强制刷新缓存
        from dashboard_cache import force_refresh_cache
        new_data = force_refresh_cache()
...
@dashboard_bp.route('/api/cache-status')
@login_required
def cache_status():
    """获取缓存状态信息的API接口"""
    try:
