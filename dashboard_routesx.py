...
def get_statistics_data():
    ...
    if has_prod_params and start_date and end_date:
        ...
        # 如果用户清空了日期，则查询所有时间
        if has_prod_params and not start_date and not end_date:
            start_month = '200001'
            end_month = '999912'
        else:
    ...
    if dimension in dimension_column_map:
        dimension_col_sql = dimension_column_map[dimension]
    elif dimension == '装机容量(万千瓦)':
        dimension_col_sql = """
            CASE 
        ...
        dimension_col_sql = "CASE WHEN p.is_uhv_support = 1 THEN '是' ELSE '否' END"
    elif dimension == '是否含补贴':
        dimension_col_sql = "CASE WHEN p.has_subsidy = 1 THEN '是' ELSE '否' END"
    elif dimension == '是否建档立卡':
        dimension_col_sql = "CASE WHEN p.is_filed = 1 THEN '是' ELSE '否' END"
    elif dimension == '是否完成北交注册':
        dimension_col_sql = "CASE WHEN p.is_beijing_registered = 1 THEN '是' ELSE '否' END"
    elif dimension == '是否完成广交注册':
        dimension_col_sql = "CASE WHEN p.is_guangzhou_registered = 1 THEN '是' ELSE '否' END"
    else:
        dimension_col_sql = 'p.province' # 默认值
    ...
    with db.engine.connect() as connection:
        ...
        base_sql = text(f"""
            SELECT
                ({dimension_col_sql}) as dimension_value,
        ...
        for row in base_results:
            dim_value = row['dimension_value'] if row['dimension_value'] is not None else '未知'
            results_agg[dim_value] = {
        ...
        # 2. 交易平台数据分别查询
        trading_platforms = [
            {'table': 'gzpt_unilateral_listings', 'qty_col': 'total_quantity', 'amt_col': 'total_amount', 'prod_col': 'generate_ym', 'trans_col': 'order_time_str', 'status_filter': "AND t.order_status = '1'"},
            {'table': 'gzpt_bilateral_online_trades', 'qty_col': 'total_quantity', 'amt_col': 'total_amount', 'prod_col': 'generate_ym', 'trans_col': 'order_time_str', 'status_filter': "AND t.order_status = '2'"},
            {'table': 'gzpt_bilateral_offline_trades', 'qty_col': 'total_quantity', 'amt_col': 'total_amount', 'prod_col': 'generate_ym', 'trans_col': 'order_time_str', 'status_filter': "AND t.order_status = '3'"},
        ...
        for platform in trading_platforms:
            ...
            for row in platform_results:
                dim_val = row['dimension_value'] if row['dimension_value'] is not None else '未知'
                if dim_val not in results_agg:
                    results_agg[dim_val] = {
                        'ordinary_total': Decimal(0),
                        'issued_platform_sold': Decimal(0),
                        'trading_platform_sold': Decimal(0),
                        'trading_platform_amount': Decimal(0)
                    }
    ...
    if summary_data:
        ...
        total_trading_amount = sum(Decimal(str(row['trading_platform_sold'])) * Decimal(str(row['avg_price'])) for row in summary_data)
        
        total_avg_price = (total_trading_amount / Decimal(total_trading_platform_sold)) if total_trading_platform_sold > 0 else 0
        ...
        summary_row = {
            ...
            'avg_price': round(float(total_avg_price), 2)
        }
