...
def statistics():
    """
    【重构后】渲染统计分析页面框架。
    数据将通过AJAX从 /api/statistics_data 接口获取。
    """
    # 获取当前日期，设置默认时间范围
    ...
    return render_template('statistics.html',
                         ...
                         transaction_end_date=request.args.get('transaction_end_date', datetime.now().strftime('%Y-%m-%d')))


@dashboard_bp.route('/api/statistics_data')
@login_required
def get_statistics_data():
    """
    【已修复】为统计分析页面提供数据的API接口。
    采用分别查询再汇总的方式，避免了扇出效应导致的数据重复计算。
    """
    current_date = datetime.now()
    default_start = f"{current_date.year}-01"
    default_end = f"{current_date.year}-{str(current_date.month).zfill(2)}"
    ...
    if not project_ids_list:
        return jsonify({'summary_data': [], 'chart_data': {'labels': [], 'issued': [], 'issued_platform_sold': [], 'trading_platform_sold': [], 'issued_ratio': [], 'avg_price': []}})

    has_prod_params = ('start_date' in request.args) or ('end_date' in request.args)
    if has_prod_params and start_date and end_date:
        start_month = start_date[:7].replace('-', '')
        end_month = end_date[:7].replace('-', '')
    else:
        start_month = default_start.replace('-', '')
        end_month = default_end.replace('-', '')
    ...
    if transaction_start_date and transaction_end_date:
        ...
        sql_params['transaction_start_date'] = transaction_start_date
        sql_params['transaction_end_date'] = end_date_for_query

    dimension_column_map = {
        '省份': 'p.province', '二级单位': 'p.secondary_unit', '电源品种': 'p.power_type',
        '项目性质': 'p.project_nature', '项目投资口径': 'p.investment_scope'
    }
    dimension_column = dimension_column_map.get(dimension, 'p.province')

    results_agg = {}

    with db.engine.connect() as connection:
        # 1. 基础数据查询 (核发量 & 核发平台售出量)
        base_sql = text(f"""
            SELECT
                {dimension_column} as dimension_value,
                COALESCE(SUM(CASE WHEN n.ordinary_quantity != '' THEN CAST(n.ordinary_quantity AS DECIMAL(15,2)) ELSE 0 END), 0) as ordinary_total,
                COALESCE(SUM(CASE WHEN tr.transaction_num != '' THEN CAST(tr.transaction_num AS DECIMAL(15,2)) ELSE 0 END), 0) as issued_platform_sold
            FROM projects p
            JOIN nyj_green_certificate_ledger n ON p.id = n.project_id
            LEFT JOIN nyj_transaction_records tr ON p.id = tr.project_id AND n.production_year_month = CONCAT(tr.production_year, LPAD(tr.production_month, 2, '0'))
                { "AND tr.transaction_time BETWEEN :transaction_start_date AND :transaction_end_date" if 'transaction_start_date' in sql_params else "" }
            WHERE p.id IN :project_ids AND n.production_year_month BETWEEN :start_month AND :end_month
            GROUP BY {dimension_column}
        """)
        base_results = connection.execute(base_sql, sql_params).mappings().all()
        for row in base_results:
            results_agg[row['dimension_value']] = {
                'ordinary_total': row['ordinary_total'],
                'issued_platform_sold': row['issued_platform_sold'],
                'trading_platform_sold': Decimal(0),
                'trading_platform_amount': Decimal(0)
            }

        # 2. 交易平台数据分别查询
        trading_platforms = [
            {'table': 'gzpt_unilateral_listings', 'qty_col': 'total_quantity', 'amt_col': 'total_amount', 'prod_col': 'generate_ym', 'trans_col': 'order_time_str', 'status_filter': "AND order_status = '1'"},
            {'table': 'gzpt_bilateral_online_trades', 'qty_col': 'total_quantity', 'amt_col': 'total_amount', 'prod_col': 'generate_ym', 'trans_col': 'order_time_str', 'status_filter': "AND order_status = '2'"},
            {'table': 'gzpt_bilateral_offline_trades', 'qty_col': 'total_quantity', 'amt_col': 'total_amount', 'prod_col': 'generate_ym', 'trans_col': 'order_time_str', 'status_filter': "AND order_status = '3'"},
            {'table': 'beijing_power_exchange_trades', 'qty_col': 'transaction_quantity', 'amt_col': 'transaction_quantity * transaction_price', 'prod_col': 'production_year_month', 'trans_col': 'transaction_time', 'status_filter': ''},
            {'table': 'guangzhou_power_exchange_trades', 'qty_col': 'gpc_certifi_num', 'amt_col': 'total_cost', 'prod_col': 'REPLACE(product_date, "-", "")', 'trans_col': 'deal_time', 'status_filter': ''}
        ]

        for platform in trading_platforms:
            trans_filter_sql = f"AND t.{platform['trans_col']} BETWEEN :transaction_start_date AND :transaction_end_date" if 'transaction_start_date' in sql_params else ""
            sql = text(f"""
                SELECT
                    p.{dimension_column} as dimension_value,
                    COALESCE(SUM(CAST(t.{platform['qty_col']} AS DECIMAL(15,2))), 0) as total_qty,
                    COALESCE(SUM(CAST({platform['amt_col']} AS DECIMAL(15,2))), 0) as total_amt
                FROM projects p
                JOIN {platform['table']} t ON p.id = t.project_id
                WHERE p.id IN :project_ids AND t.{platform['prod_col']} BETWEEN :start_month AND :end_month
                {trans_filter_sql} {platform['status_filter']}
                GROUP BY p.{dimension_column}
            """)
            platform_results = connection.execute(sql, sql_params).mappings().all()
            for row in platform_results:
                dim_val = row['dimension_value']
                if dim_val in results_agg:
                    results_agg[dim_val]['trading_platform_sold'] += row['total_qty']
                    results_agg[dim_val]['trading_platform_amount'] += row['total_amt']

    # 3. 汇总和格式化最终结果
    summary_data = []
    chart_data = {'labels': [], 'issued': [], 'issued_platform_sold': [], 'trading_platform_sold': [], 'issued_ratio': [], 'avg_price': []}

    for dim_val, data in results_agg.items():
        ordinary_total = data['ordinary_total']
        issued_platform_sold = data['issued_platform_sold']
        trading_platform_sold = data['trading_platform_sold']
        trading_platform_amount = data['trading_platform_amount']

        avg_price = (trading_platform_amount / trading_platform_sold) if trading_platform_sold > 0 else Decimal(0)
        issued_ratio = (issued_platform_sold / ordinary_total * 100) if ordinary_total > 0 else Decimal(0)
        trading_ratio = (trading_platform_sold / ordinary_total * 100) if ordinary_total > 0 else Decimal(0)

        summary_data.append({
            'dimension_value': dim_val or '未知',
            'ordinary_total': int(ordinary_total),
            'issued_platform_sold': int(issued_platform_sold),
            'trading_platform_sold': int(trading_platform_sold),
            'issued_ratio': round(float(issued_ratio), 1),
            'trading_ratio': round(float(trading_ratio), 1),
            'avg_price': round(float(avg_price), 2)
        })

    # 排序
    summary_data.sort(key=lambda x: x['ordinary_total'], reverse=True)

    for item in summary_data:
        chart_data['labels'].append(item['dimension_value'])
        chart_data['issued'].append(round(item['ordinary_total'] / 10000, 1))
        chart_data['issued_platform_sold'].append(round(item['issued_platform_sold'] / 10000, 1))
        chart_data['trading_platform_sold'].append(round(item['trading_platform_sold'] / 10000, 1))
        chart_data['issued_ratio'].append(item['issued_ratio'])
        chart_data['avg_price'].append(item['avg_price'])
    ...
    if summary_data:
        ...
        total_trading_amount = sum(row['trading_platform_sold'] * row['avg_price'] for row in summary_data)
        
        total_avg_price = (total_trading_amount / total_trading_platform_sold) if total_trading_platform_sold > 0 else 0
        total_issued_ratio = (total_issued_platform_sold / total_ordinary * 100) if total_ordinary > 0 else 0
        total_trading_ratio = (total_trading_platform_sold / total_ordinary * 100) if total_ordinary > 0 else 0
        ...
        summary_row = {
            ...
            'issued_ratio': round(total_issued_ratio, 1),
            'trading_ratio': round(total_trading_ratio, 1),
            'avg_price': round(total_avg_price, 2)
        }
        summary_data.insert(0, summary_row)

    return jsonify({'summary_data': summary_data, 'chart_data': chart_data})
...
def get_transaction_time_data():
    ...
    with db.engine.connect() as connection:
        ...
        time_conditions = []
        if transaction_start_date:
            time_conditions.append("AND bj.transaction_time >= :transaction_start_date")
            main_sql_params['transaction_start_date'] = transaction_start_date
        if transaction_end_date:
            time_conditions.append("AND bj.transaction_time <= :transaction_end_date")
            main_sql_params['transaction_end_date'] = transaction_end_date
            
        time_condition_str = ' '.join(time_conditions)
        ...
        transaction_months_sql = text(f"""
            ...
            WHERE bj.project_id IN :project_ids AND bj.transaction_time IS NOT NULL {time_condition_str}
            UNION
            SELECT DISTINCT SUBSTRING(gz.deal_time, 1, 7)
            FROM guangzhou_power_exchange_trades gz
            WHERE gz.project_id IN :project_ids AND gz.deal_time IS NOT NULL {time_condition_str.replace('bj.', 'gz.').replace('transaction_date', 'transaction_date')}
            UNION
            SELECT DISTINCT SUBSTRING(ul.order_time_str, 1, 7)
            FROM gzpt_unilateral_listings ul
            WHERE ul.project_id IN :project_ids AND ul.order_time_str IS NOT NULL {time_condition_str.replace('bj.', 'ul.').replace('transaction_date', 'order_time_str')}
            UNION
            SELECT DISTINCT SUBSTRING(ol.order_time_str, 1, 7)
            FROM gzpt_bilateral_online_trades ol
            WHERE ol.project_id IN :project_ids AND ol.order_time_str IS NOT NULL {time_condition_str.replace('bj.', 'ol.').replace('transaction_date', 'order_time_str')}
            UNION
            SELECT DISTINCT SUBSTRING(off.order_time_str, 1, 7)
            FROM gzpt_bilateral_offline_trades off
            WHERE off.project_id IN :project_ids AND off.order_time_str IS NOT NULL {time_condition_str.replace('bj.', 'off.').replace('transaction_date', 'order_time_str')}
        ...
        for month_row in months_result:
            ...
            # 添加时间筛选条件
            if transaction_start_date:
                guangzhou_sql_conditions.append("product_date >= :transaction_start_date")
                guangzhou_sql_params['transaction_start_date'] = transaction_start_date
            if transaction_end_date:
                guangzhou_sql_conditions.append("product_date <= :transaction_end_date")
                guangzhou_sql_params['transaction_end_date'] = transaction_end_date
            if production_start_month:
                guangzhou_sql_conditions.append("product_date >= :production_start_month")
                guangzhou_sql_params['production_start_month'] = production_start_month
            if production_end_month:
                guangzhou_sql_conditions.append("product_date <= :production_end_month")
...
def get_dashboard_chart_data():
    """
    【已修复并补全】获取数据概览页面图表所需的真实数据。
    为'双边线上'交易添加了 order_status = '2' 的过滤条件。
    """
    query = Project.query
    if not current_user.is_admin:
        query = query.filter(Project.secondary_unit == current_user.username)
        ...
        for month in transaction_months:
            ...
            for prod_year in production_years:
                ...
                try:
                    ...
                    trans_end = f"{month}-{last_day} 23:59:59"
                except ValueError:
                ...
                sql_params = {
                    'project_ids': tuple(project_ids_list),
                    'prod_start': prod_start.replace('-', ''),
                    'prod_end': prod_end.replace('-', ''),
                ...
                unilateral_sql = text("""
                    SELECT 
                        COALESCE(SUM(CASE WHEN total_quantity != '' THEN CAST(total_quantity AS DECIMAL(15,2)) ELSE 0 END), 0) as qty,
                        COALESCE(SUM(CASE WHEN total_amount != '' THEN CAST(total_amount AS DECIMAL(15,2)) ELSE 0 END), 0) as amt
                    FROM gzpt_unilateral_listings 
                    WHERE project_id IN :project_ids AND generate_ym BETWEEN :prod_start AND :prod_end
                    AND order_time_str BETWEEN :trans_start AND :trans_end AND order_status = '1'
                """)
                result = connection.execute(unilateral_sql, sql_params).fetchone()
                ...
                offline_sql = text("""
                    ...
                    FROM gzpt_bilateral_offline_trades 
                    WHERE project_id IN :project_ids AND generate_ym BETWEEN :prod_start AND :prod_end
                    AND order_time_str BETWEEN :trans_start AND :trans_end AND order_status = '3'
                """)
                result = connection.execute(offline_sql, sql_params).fetchone()
                total_qty += float(result[0] or 0)
                total_amt += float(result[1] or 0)

                # 3. 绿证平台-双边线上 (FIXED: Added order_status = '2')
                online_sql = text("""
                    SELECT 
                        COALESCE(SUM(CASE WHEN total_quantity != '' THEN CAST(total_quantity AS DECIMAL(15,2)) ELSE 0 END), 0) as qty,
                        COALESCE(SUM(CASE WHEN total_amount != '' THEN CAST(total_amount AS DECIMAL(15,2)) ELSE 0 END), 0) as amt
                    ...
                    AND order_status = '2'
                """)
                result = connection.execute(online_sql, sql_params).fetchone()
                total_qty += float(result[0] or 0)
                total_amt += float(result[1] or 0)
                ...
                beijing_sql = text("""
                    SELECT 
                        COALESCE(SUM(CASE WHEN transaction_quantity != '' THEN CAST(transaction_quantity AS DECIMAL(15,2)) ELSE 0 END), 0) as qty,
                ...
                """)
                result = connection.execute(beijing_sql, sql_params).fetchone()
                total_qty += float(result[0] or 0)
                total_amt += float(result[1] or 0)
                ...
                guangzhou_sql = text("""
                    SELECT 
                        COALESCE(SUM(CASE WHEN gpc_certifi_num != 0 THEN CAST(gpc_certifi_num AS DECIMAL(15,2)) ELSE 0 END), 0) as qty,
                        COALESCE(SUM(CASE WHEN total_cost != 0 THEN CAST(total_cost AS DECIMAL(15,2)) ELSE 0 END), 0) as amt
                    FROM guangzhou_power_exchange_trades
                    WHERE project_id IN :project_ids
                    AND REPLACE(product_date, '-', '') BETWEEN :prod_start AND :prod_end
                    AND deal_time BETWEEN :trans_start AND :trans_end
                """)
                result = connection.execute(guangzhou_sql, sql_params).fetchone()
                total_qty += float(result[0] or 0)
                total_amt += float(result[1] or 0)
